...
#include <sched.h>                     // System scheduling API
#endif
...
#include <omp.h>                       // OpenMP library (for timing)
...
typedef struct tr{ 
  ...
  tr_t*     trs;                       // array of tr_t objects
} tr_t;
...
int setup_from_args(tr_t* t, int argc, char** argv){
  ...
  if(t->P > how_many_cores()){
    stderr_out("Too many processors! (only %u cores available)\n",
               how_many_cores());
    goto err;
  }
...
int initialize_data(tr_t* t){
  ...
  // - P tr_t objects (one for each thread).
  //  (Note the scratchpads are taken care of within each thread)
  ...
  // And initialize the array of tr_t objects with copies of this object,
  // each containing the 'right' value of Pi.
...
void cleanup_data(tr_t* t){
  ...
  if(t->trs != NULL){ free(t->trs);  t->trs = NULL; }
...
void show_usage(){
  print_out("\nusage:\n"
            "  fourier-parallel-pi-cpu-pthreads { -n <n> -p <p> [-o] | -t }\n"
            "\noptions:\n"
...
// 
int run(tr_t* t){
  pthread_attr_t attr;                      // pthread_attr_t obj
#ifndef __APPLE__
  cpu_set_t* cpu;                           // cpu bitmask
                                            //  (for pinning threads to cpus)
  
  // Allocate the cpu_set_t object
  // From: http://man7.org/linux/man-pages/man3/CPU_SET.3.html
  if((cpu = CPU_ALLOC(t->P)) == NULL){
    perror_out("CPU_ALLOC error");
    return -1;
  }
  CPU_ZERO_S(CPU_ALLOC_SIZE(t->P), cpu);
#endif
  ...
  // Spawn P pthreads, each running 'run_thread' (or return -1 if err)
  for(t->Pi = 0; t->Pi < t->P; t->Pi++){
#ifndef __APPLE__
    // Pin the thread to the given core
    CPU_SET_S(bit_reverse(t->Pi, ilog2(t->P)), CPU_ALLOC_SIZE(t->P), cpu);  
    if(pthread_attr_setaffinity_np(&attr, CPU_ALLOC_SIZE(t->P), cpu)){ 
      perror_out("pthread_attr_setaffinity_np error");
      CPU_FREE(cpu);
      return -1;
    }
#endif
    if(pthread_create(&t->trs[t->Pi].tid, &attr, run_thread, t->trs+t->Pi)){ 
      perror_out("pthread_create error");
      cleanup_data(t);
      return -1;
    }
  }
  ...
  // Join the threads
  for(t->Pi = 0; t->Pi < t->P; t->Pi++){
    if(pthread_join(t->trs[t->Pi].tid, NULL)){ 
      perror_out("pthread_join error");
      cleanup_data(t);
      return -1;
    }
  }
...
// This function runs the given transform on the assigned processor.
//
//   t_ptr:                  (ptr to) tr_t object to update
//
//   returns:                NULL always.
// 
void* run_thread(void* t_ptr){
  ...
  tr_t t = *((tr_t*)t_ptr);                 // local, unpacked tr_t object
  ...
  // Start the timer for the tree stage
  ...
  // Start the timer for the cylinder stage
  timer_start(&tm_tube);
  ...
  // Stop the timer for the cylinder stage
  timer_stop(&tm_tube);
...
err:
  stderr_out("(Processor %u): Could not run the transform\n", t.Pi);
  return NULL;
}
...
// This function returns the number of cores in the CPU.
//
