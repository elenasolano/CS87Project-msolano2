...
if(!file.exists("fourier-parallel-pi-cpu-pthreads-results.csv")){
  cat("Sorry, could not find fourier-parallel-pi-cpu-pthreads-results.csv!\n");
  quit();
}

# Read in the results
r = read.table(file   = "fourier-parallel-pi-cpu-pthreads-results.csv", 
               header = FALSE, 
               sep    = "\t",
...
for(ni in n_plot){
  invisible(pdf(width=14, height=7, file=sprintf(
    "fourier-parallel-pi-cpu-pthreads-results-analysis-n%d.pdf", ni)));
  r_sub = r[r$n==ni,];
  ...
  plot(0, pch='', 
    ...
    "Empirical speedup of pi-DFT on multicore CPU (%d replications)\n",
  ...
  barplot(col=c("#629627", "#623034"),
    height=rbind(avg_time_tr(ni, p, r),
                 avg_time_cy(ni, p, r)),
    main=sprintf(paste(sep="",
    "Average time of pi-DFT on multicore CPU (%d replications)\n",
...
cat(sep="",
  "Done. See also:\n", paste(
  "  fourier-parallel-pi-cpu-pthreads-results-analysis-n",n_plot,".pdf\n",
  sep="", collapse=""));
